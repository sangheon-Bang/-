...
public class Seat_pan extends Seat_panAb implements ActionListener,
	...
	public Seat_pan(int i) {
		...
		for (int a = 0; a < 4; a++) {
			switch(a) {
			case 0:
				String s=(num/10+1)*100+num%10/2+1 + " - ";
				s+=num%10%2+1;
				label[a] = new JLabel(s);
				label[a].setForeground(Color.black);
				break;
			case 1:
				switch(i/10) {
				case 0:
					label[a] = new JLabel("VIP");
					label[a].setForeground(Color.CYAN);
					break;
				case 1:
				case 2:
					label[a] = new JLabel("Premium");
					label[a].setForeground(Color.magenta);
					break;
				case 3:
				case 4:
					label[a] = new JLabel("Deluxe");
					label[a].setForeground(Color.BLUE);
					break;
				}
				break;
			default:
