...
public class ClientPc {// 클라이언트 클래스 시작
	...
	ClientPc(String pc) {// 클라이언트 생성자시작
		...
		pc_label.setFont(new Font("맑은 고딕",Font.BOLD,20));
		userPc.setFont(new Font("맑은 고딕",Font.BOLD,20));
		JLabel time_label = new JLabel(" 퇴실 시간:");
		time_label.setFont(new Font("맑은 고딕",Font.BOLD,20));
		userTime.setFont(new Font("맑은 고딕",Font.BOLD,20));
		JLabel label0 = new JLabel(" ");

		// 버튼
		JButton chatBtn = new JButton("  채팅  ");
		chatBtn.setFont(new Font("맑은 고딕",Font.BOLD,20));

		// 컴포넌트가 붙을 패널 생성
		JPanel timepanel = new JPanel();
		JPanel pcnumpanel = new JPanel();
		...
		// 컴포넌트 배치부
		userPc.setPreferredSize(new Dimension(95,30));
		userTime.setPreferredSize(new Dimension(95,30));

		// 컴포넌트 결합부
		pcnumpanel.add(pc_label);
		timepanel.add(time_label);
		pcnumpanel.add(userPc);
		timepanel.add(userTime);
		...
		panel.setLayout(b);
		panel.add(pcnumpanel);
		panel.add(timepanel);
		panel.add(chatpanel);
		panel.add(label0);
		npanel.setLayout(new BorderLayout());
		npanel.add(panel,"West");
		clFrame.add(npanel,"North");
		clFrame.add(imgpanel,"Center");
